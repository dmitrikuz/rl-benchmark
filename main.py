...
parser.add_argument('-e', '--environment', help='Environment index', default=1)
parser.add_argument('-m', '--metrics', help='Metrics to be saved and displayed on plot', nargs='+', default=['rewards', 'paths'])
parser.add_argument('-p', '--epsilon', help='Randomness parameter', default=0.05)
...
def do_plots(env_dicts, prefix='', epochs=NUM_EPOCHS):
    ...
    for env_dict, color in zip(env_dicts, colors):
        col = (np.random.random(), np.random.random(), np.random.random())
        rewards_splitted = splitted(env_dict['env'].path_lengths)
        plt.plot(x, rewards_splitted, label=env_dict['name'], color=color)
...
def run():
    ...
    if 'Q-learning' in args.algorithms:
        ...
        qlearning_agent = QLearningAgent(name='Q-Learning', alpha=LR, epsilon=0.05, discount=0.99, possible_actions=range(3))
    ...
    if 'SARSA' in args.algorithms:
        ...
        sarsa_agent = QLearningAgent(name='SARSA', alpha=LR, epsilon=0.05, discount=0.99, possible_actions=range(3))
    ...
    if 'Actor-Critic' in args.algorithms:
        print("Actor-Critic")
        ac_env = gym.make(ENV_NAME, disable_env_checker=True, old=True, flat=True)
    ...
    if 'REINFORCE' in args.algorithms:
        print('REINFORCE')
        reinforce_env = gym.make(ENV_NAME, disable_env_checker=True, old=True, flat=True)
    ...
    if 'D3QN' in args.algorithms:
        ...
        to_plot.append({'env': d3qn_env, 'name': 'DQN'})
